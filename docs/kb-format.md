...
A Published Version may carry marked Navigation and Hot Index artifacts beside
...
fingerprint. Direct edits are overwritten on the next publish.
...
---
lumio:
  artifact: navigation-index   # or hot-index
  version: 1
---
```
...
**Legacy Activity Log** (`log.md`) is recognized only so older Published
Versions continue to load and fingerprint identically. A valid historical file
must carry `lumio.artifact: activity-log`; new publications do not generate or
append it. Unmarked or malformed `log.md` collisions remain blocking
(ADR-0022).
