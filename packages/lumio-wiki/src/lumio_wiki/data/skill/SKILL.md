...
| `lumio-wiki dream <kb> [--limit N] [--stage] [--semantic] [--impact T] [--impact-scope S]` | Deterministic Dream Cycle reflection (health, structure, ranked link candidates, due pages, Source Drift, Source Coverage) plus optional semantic review; `--semantic` requires the `[llm]` extra and remains proposal-first. `synthesis_candidates` lists read-only page pairs whose accepted Claims justify one synthetic page (mutual claims, one-way claim, shared Claim object) — advisory only. `--impact <title-or-entity>` adds the bounded transitive-impact report for ONE page. |
...
4. Weigh synthesis candidates: `synthesis_candidates` in the dream report
   ...
   dangling targets never justify a pair. For each pair you agree is worth
   synthesizing, author the synthetic page YOURSELF (you are the Distiller)
   and stage it through the ordinary `ingest`/proposal pipeline. Nothing is
   authored or staged automatically (ADR-0021).
5. Inspect one page's reach: `lumio-wiki dream <kb> --impact "<title or
   ...
   reachable from it, grouped by hop distance. Use it to see what a page
   change touches before editing. `--impact-scope discovery` widens
   traversal with Extracted References (body links); discovery edges
   select pages to inspect only — they are never Evidence and never
   support a conclusion. The report is bounded (`max_depth` 3,
   `max_pages` 100) and ranks nothing across the Knowledge Base.
6. `lumio-wiki dream <kb> --stage [--limit N]` — stage the top repairs as
...
7. `lumio-wiki cross-link <kb>` is the focused variant when you only want
...
8. Canonical edges are accepted, evidence-bearing **Claims** authored directly
