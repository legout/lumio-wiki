"""S3-compatible integration coverage for remote LanceDB indexes (issue #122, ADR-0013).
...
``lumio-wiki`` S3-compatible suite.
"""
...
if not _os.environ.get("LUMIO_S3_ENDPOINT"):  # pragma: no cover
    pytest.skip(
        "LUMIO_S3_ENDPOINT not set; skipping remote LanceDB S3-compatible tests",
...
class _FakeEmbedder:
    ...
    def __init__(self, name="s3_compat-fake", dimension=8):
...
def remote_location():
    """Build a RemoteIndexLocation rooted at a unique test prefix in S3-compatible."""
...
def test_s3_compat_remote_lexical_build_and_search(remote_location):
    loc, store, prefix = remote_location
    pages = _pages()
    fp = SourceFingerprint(digest="s3_compat-canonical-fingerprint")
...
def test_s3_compat_remote_fingerprint_sidecar_is_canonical(remote_location):
    ...
    assert loaded.digest == "s3_compat-canonical-fingerprint"
    ...
    assert (
        ...
        == "s3_compat-canonical-fingerprint"
    )


def test_s3_compat_remote_semantic_build_search_and_identity(remote_location):
    loc, store, prefix = remote_location
    pages = _pages()
    embedder = _FakeEmbedder()
    fingerprint = SourceFingerprint(digest="s3_compat-semantic-fingerprint")

    # Build the semantic vector index beside the lexical one.
    adapter = LanceDBRetrievalAdapter()
    adapter.build_index(pages, loc, embedder=embedder, fingerprint=fingerprint)
...
def test_s3_compat_remote_model_mismatch_is_rejected(remote_location):
    loc, store, prefix = remote_location
    pages = _pages()
    adapter = LanceDBRetrievalAdapter()
    fingerprint = SourceFingerprint(digest="s3_compat-model-mismatch-fingerprint")
    adapter.build_index(pages, loc, embedder=_FakeEmbedder(name="model-a"), fingerprint=fingerprint)
...
def test_s3_compat_missing_remote_index_falls_back_to_zero_index(remote_location):
...
def test_s3_compat_remote_graph_tables_build_load_and_parity(remote_location):
